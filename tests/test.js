...
describe('electron-stats-server (ESS) tests', () => {
  ...
  describe('.env ( Class:DotEnv )', () => {
    ...
    it('dotEnv: string - .env File Path', () => {
      denv.dotEnv;
    });
    it('dotEnvBak: string - .env.bak File Path', () => {
      denv.dotEnvBak;
    });
    it('dotEnvTemplate: string - .envTemplate Path', () => {
      denv.dotEnvTemplate;
    });
  ...
  describe('db => Authentication ( Class:AuthDB )', () => {
    ...
    it('async add() => Add application authentication key', async () => {
      ...
      let salted = await scrypt.kdf(new Buffer(authPrivateKey), {
        N: 1,
        r: 1,
        p: 1
      });
      ...
      let test = await authDB.add(appid, salted.toString('base64'), 365);
      should(test).be.True();
    });
